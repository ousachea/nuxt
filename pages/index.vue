      ...
      <div class="desktop-grid">
        ...
        <div class="col-left">
          ...
          <section class="card price-hero">
            ...
            <div class="seg-ctrl">
              ...
              <button class="seg-btn" :class="{ active: priceSource === 'custom' }" @click="priceSource = 'custom'">
                ✦ {{ t.custom }}
              </button>
            </div>
              ...
              <div class="hero-meta-row">
                ...
                <div style="display:flex;align-items:center;gap:6px;">
                  <transition name="fade">
                    <span v-if="priceTickDir" class="tick-arrow" :class="priceTickDir">{{ priceTickDir === 'up' ? '▲' : '▼' }}</span>
                  </transition>
                  <span v-if="lastUpdated && priceSource === 'api'" class="last-updated">{{ lastUpdated }}</span>
                </div>
              </div>
              <transition name="price-flip" mode="out-in">
                <div class="hero-price" :class="priceTickDir ? `price-tick-${priceTickDir}` : ''" :key="displayPrice?.toFixed(0) ?? 'null'">
                  <span class="price-dollar">$</span>
                  <span class="price-int">{{ animatedPrice ? Math.floor(animatedPrice).toLocaleString() : '——' }}</span>
                  <span v-if="displayPrice" class="price-dec">.{{ (displayPrice % 1).toFixed(2).slice(2) }}</span>
                </div>
              </transition>
            ...
            <div v-if="priceHistory.length >= 2" class="sparkline-wrap">
              <svg class="sparkline" viewBox="0 0 280 40" preserveAspectRatio="none">
                ...
                <path :d="sparklinePath" fill="none" :stroke="sparklineColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            ...
            <div v-if="priceSource === 'custom'" class="sub-panel">
              ...
              <div class="price-input-row">
                ...
                <input v-model.number="customPrice" class="text-input price-input" type="text" inputmode="decimal"
        ...
        <div class="col-center">
          ...
          <section class="card">
            ...
            <div v-if="displayPrice && convInput && convInput !== 1 && convValueUSD" class="conv-total-tile">
              <span class="conv-total-label">Total value of {{ convInput }} {{ t[activeConv] || activeConv }}</span>
              ...
              <transition name="slide-down">
                <div v-if="showForm" class="purchase-form">
                  <div class="form-grid">
                    ...
                    <div class="form-field">
                      <label>{{ t.date }}</label>
                      <input v-model="draft.date" type="date" class="text-input" :placeholder="today()" />
                      <button v-if="draft.date !== today()" type="button" class="today-chip" @click="draft.date = today()">↺ Today</button>
                    </div>
                  </div>
          ...
          <section class="card summary-card" v-if="!purchasesLocked && purchases.length">
            ...
            <div class="kpi-stack">
              ...
              <div class="kpi-item kpi-big" :class="totalGL >= 0 ? 'kpi-gain' : 'kpi-loss'">
                ...
                <span class="kpi-val kpi-main">{{ displayGL >= 0 ? '+' : '' }}${{ displayGL.toFixed(2) }}</span>
              </div>
            </div>
...
.orb-1 { width: 500px; height: 500px; background: #F5C842; top: -150px; right: -100px; animation: orbDrift1 16s ease-in-out infinite alternate; }
.orb-2 { width: 300px; height: 300px; background: #A07020; bottom: 20%; left: -80px; animation: orbDrift2 20s ease-in-out infinite alternate-reverse; }
.orb-3 { width: 200px; height: 200px; background: #F5A623; bottom: 10%; right: 15%; animation: orbDrift3 24s ease-in-out infinite alternate; }
...
.logo-gem { font-size: 22px; color: var(--gold); flex-shrink: 0; line-height: 1; display: inline-block; animation: gemPulse 4s ease-in-out infinite; }
@keyframes gemPulse {
...
.logo-title { font-size: 17px; font-weight: 800; letter-spacing: -0.3px; white-space: nowrap; }
.logo-sub { font-size: 10px; color: var(--text-3); letter-spacing: 0.05em; margin-top: 1px; }
...
.lang-btn { font-size: 12px; padding: 0 12px; min-width: unset; }
...
.sticky-val { font-size: 22px; font-weight: 800; color: var(--gold); font-family: var(--mono); letter-spacing: -0.5px; }
.sticky-unit { font-size: 12px; color: var(--text-3); }
.sticky-val-sub { font-size: 14px; font-weight: 700; color: var(--gold-dim); font-family: var(--mono); }
.sticky-unit-sub { font-size: 11px; color: var(--text-3); }
.sticky-divider { width: 1px; height: 32px; background: var(--border); flex-shrink: 0; }
.sticky-refresh { background: none; border: none; color: var(--text-2); cursor: pointer; font-size: 20px; padding: 4px; transition: color 0.2s; margin-left: auto; }
...
.hero-meta-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }
.metal-tag { font-size: 12px; font-weight: 600; color: var(--text-2); letter-spacing: 0.04em; }
.last-updated { font-size: 11px; color: var(--text-3); font-family: var(--mono); }
.tick-arrow { font-size: 11px; font-weight: 700; font-family: var(--mono); }
...
.price-dollar { font-size: 24px; font-weight: 800; color: var(--gold); align-self: flex-start; margin-top: 4px; }
.price-int { font-size: clamp(40px, 10vw, 64px); font-weight: 800; color: var(--gold); letter-spacing: -2px; font-variant-numeric: tabular-nums; }
.price-dec { font-size: 20px; font-weight: 700; color: var(--gold-dim); }
.price-unit-label { font-size: 12px; color: var(--text-3); font-family: var(--mono); }
...
.chip-label { font-size: 10px; font-weight: 700; color: var(--text-3); text-transform: uppercase; letter-spacing: 0.06em; }
.chip-price { font-size: 12px; font-weight: 600; color: var(--text); font-family: var(--mono); }

/* ── Progress / Flash ── */
.progress-bar { height: 3px; background: var(--surface3); border-radius: 2px; overflow: hidden; margin: 10px 0; }
.progress-fill { height: 100%; background: var(--gold); animation: progress 1.4s ease-in-out infinite; }
@keyframes progress {
...
.flash { padding: 10px 14px; border-radius: 10px; font-size: 13px; font-weight: 500; margin-top: 10px; }
.flash.success { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); color: var(--gain); }
.flash.error { background: rgba(248, 113, 113, 0.1); border: 1px solid rgba(248, 113, 113, 0.3); color: var(--loss); }

/* ── Sub panel ── */
.sub-panel { margin-top: 14px; padding-top: 14px; border-top: 1px solid var(--border); }
.sub-panel-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; margin-bottom: 10px; flex-wrap: wrap; }
.api-info { display: flex; flex-direction: column; gap: 3px; flex: 1; min-width: 0; }
.api-badge { display: inline-block; width: fit-content; font-size: 10px; font-weight: 700; letter-spacing: 0.05em; background: var(--gold-alpha); border: 1px solid var(--gold-border); color: var(--gold); padding: 3px 8px; border-radius: 6px; }
.api-hint-text { font-size: 12px; color: var(--text-3); }
.api-key-row { display: flex; gap: 6px; margin-bottom: 8px; }
.api-hint { font-size: 12px; color: var(--text-3); line-height: 1.5; }
...
.method-btn { flex: 1; padding: 10px 8px; border: 1px solid var(--border); border-radius: 8px; background: var(--surface2); color: var(--text-2); font-family: var(--font); font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
.method-btn.active { background: var(--gold-alpha); border-color: var(--gold-border); color: var(--gold); }

.price-input-row { display: flex; }
.input-prefix { background: var(--surface3); border: 1px solid var(--border); border-right: none; border-radius: 10px 0 0 10px; padding: 14px 14px; font-size: 20px; color: var(--gold); font-family: var(--mono); display: flex; align-items: center; flex-shrink: 0; }
...
.icon-btn-sm { background: var(--surface2); border: 1px solid var(--border); color: var(--text-2); border-radius: 10px; font-size: 16px; cursor: pointer; min-height: var(--touch); min-width: var(--touch); display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s; flex-shrink: 0; }
...
/* ── Section titles ── */
.section-title { font-size: 13px; font-weight: 700; letter-spacing: 0.02em; color: var(--text); margin-bottom: 14px; }
.section-title::before { content: '◈ '; color: var(--gold); font-size: 10px; }
.section-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; gap: 8px; flex-wrap: wrap; }
.section-header .section-title { margin-bottom: 0; }
.section-actions { display: flex; gap: 6px; align-items: center; }
.lock-btn { padding: 8px 10px; font-size: 14px; }

/* ── Converter ── */
.conv-tabs-scroll { display: flex; gap: 5px; overflow-x: auto; -webkit-overflow-scrolling: touch; scrollbar-width: none; padding-bottom: 2px; margin-bottom: 12px; }
.conv-tabs-scroll::-webkit-scrollbar { display: none; }
.conv-tab { padding: 8px 12px; border: 1px solid var(--border); background: var(--surface2); color: var(--text-2); font-family: var(--font); font-size: 12px; font-weight: 600; border-radius: 8px; white-space: nowrap; cursor: pointer; transition: all 0.2s; flex-shrink: 0; }
.conv-tab.active { background: var(--gold-alpha); border-color: var(--gold-border); color: var(--gold); }
.conv-input-row { display: flex; gap: 0; margin-bottom: 10px; }
.from-badge { background: var(--gold-alpha); border: 1px solid var(--gold-border); border-right: none; border-radius: 10px 0 0 10px; padding: 12px 12px; color: var(--gold); font-size: 12px; font-weight: 700; display: flex; align-items: center; white-space: nowrap; flex-shrink: 0; }
.conv-input { border-radius: 0 10px 10px 0 !important; font-size: 16px !important; font-family: var(--mono) !important; }
.conv-results { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
.conv-row { display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; border-bottom: 1px solid var(--border); gap: 8px; }
...
.unit-tile { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; padding: 12px 10px; display: flex; flex-direction: column; gap: 6px; transition: border-color 0.2s; }
.unit-tile:hover { border-color: var(--gold-border); }
.tile-top { display: flex; justify-content: space-between; align-items: center; }
.tile-name { font-size: 10px; font-weight: 700; color: var(--text-2); text-transform: uppercase; letter-spacing: 0.06em; }
.tile-gram { font-size: 10px; color: var(--text-3); font-family: var(--mono); }
.tile-price { font-size: 14px; font-weight: 700; color: var(--text); font-family: var(--mono); font-variant-numeric: tabular-nums; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

/* ── Password Gate ── */
.pw-gate { display: flex; justify-content: center; padding: 16px 0 8px; }
.pw-box { display: flex; flex-direction: column; gap: 10px; width: 100%; max-width: 340px; align-items: stretch; }
.pw-icon { font-size: 32px; text-align: center; margin-bottom: 4px; }
.pw-title { font-size: 15px; font-weight: 700; color: var(--text); text-align: center; margin-bottom: 4px; line-height: 1.4; }
.pw-sub { font-size: 12px; color: var(--text-3); text-align: center; line-height: 1.5; margin-bottom: 4px; }
.pw-error { font-size: 13px; color: var(--loss); text-align: center; font-weight: 500; }

/* ── Lock ripple rings ── */
.lock-ripple-wrap { position: relative; display: inline-flex; align-items: center; justify-content: center; width: 52px; height: 52px; }
.lock-ripple {
...
/* ── Add purchase ── */
.add-purchase-btn { display: flex; align-items: center; justify-content: center; gap: 10px; width: 100%; background: var(--surface2); border: 1.5px dashed var(--border-hi); border-radius: 14px; color: var(--text-2); font-family: var(--font); font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; padding: 14px 20px; margin-bottom: 12px; }
.add-purchase-btn:hover { border-color: var(--gold-border); color: var(--gold); background: var(--gold-alpha); }
.add-icon { font-size: 20px; line-height: 1; }

.purchase-form { background: var(--surface2); border: 1px solid var(--border); border-radius: 14px; padding: 14px; margin-bottom: 12px; }
.form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 12px; }
.form-field { display: flex; flex-direction: column; gap: 5px; }
.form-field label { font-size: 11px; font-weight: 700; color: var(--text-2); text-transform: uppercase; letter-spacing: 0.07em; }

/* ── Purchase cards ── */
.purchases-list { display: flex; flex-direction: column; gap: 8px; margin-bottom: 14px; }
.p-card { background: var(--surface2); border: 1px solid var(--border); border-radius: 14px; padding: 14px; transition: all 0.2s; border-left: 3px solid var(--border); }
.pcard-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px; gap: 8px; }
.pcard-weight-row { display: flex; flex-direction: column; gap: 3px; min-width: 0; }
.pcard-weight { font-size: 16px; font-weight: 800; color: var(--text); font-variant-numeric: tabular-nums; }
...
.pcard-btn { background: var(--surface3); border: 1px solid var(--border); color: var(--text-2); border-radius: 8px; font-size: 13px; cursor: pointer; min-height: 36px; min-width: 36px; transition: all 0.2s; display: inline-flex; align-items: center; justify-content: center; }
...
.gl-col { flex: 1; display: flex; flex-direction: column; gap: 3px; padding: 0 4px; }
.gl-col:first-child { padding-left: 0; }
.gl-col:last-child { padding-right: 0; }
.gl-divider { width: 1px; background: var(--border); flex-shrink: 0; margin: 0 4px; }
.gl-label { font-size: 10px; font-weight: 600; color: var(--text-3); text-transform: uppercase; letter-spacing: 0.05em; }
...
.kpi-stack { display: flex; flex-direction: column; gap: 8px; margin-bottom: 14px; }
...
.kpi-val { font-size: 18px; font-weight: 800; color: var(--text); font-family: var(--mono); font-variant-numeric: tabular-nums; }
...
.portfolio-bar { height: 5px; background: var(--surface3); border-radius: 3px; overflow: hidden; margin-bottom: 6px; }
...
/* ── Locked placeholder ── */
.locked-placeholder { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 24px 16px; background: var(--surface2); border-radius: 12px; border: 1.5px dashed var(--border); }
.locked-icon { font-size: 28px; color: var(--text-3); }
.locked-placeholder p { font-size: 12px; color: var(--text-3); text-align: center; }

/* ── Holdings table ── */
.holdings-table { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
.ht-header { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 0; padding: 8px 12px; background: var(--surface3); border-bottom: 1px solid var(--border); }
.ht-header span { font-size: 10px; font-weight: 700; color: var(--text-3); text-transform: uppercase; letter-spacing: 0.06em; }
.ht-row { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 0; padding: 9px 12px; border-bottom: 1px solid var(--border); align-items: center; font-size: 12px; font-family: var(--mono); font-weight: 600; transition: background 0.15s; }
...
/* ── Quick Reference ── */
.qref-list { display: flex; flex-direction: column; gap: 0; background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
.qref-row { display: flex; justify-content: space-between; align-items: center; padding: 9px 14px; border-bottom: 1px solid var(--border); }
...
/* ── Empty state ── */
.empty-state { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 32px 20px; background: var(--surface2); border-radius: 12px; border: 1.5px dashed var(--border); }
...
/* ── Footer ── */
.footer { text-align: center; padding: 20px 16px; font-size: 11px; color: var(--text-3); font-weight: 500; border-top: 1px solid var(--border); }
...
.sparkline-low, .sparkline-high { font-size: 10px; font-family: var(--mono); color: var(--text-3); }
...
.conv-usd-label { font-size: 11px; font-weight: 700; color: var(--gold); text-transform: uppercase; letter-spacing: 0.06em; }
...
/* ── Transitions ── */
@keyframes priceFlipOut { to { opacity: 0; transform: translateY(10px) rotateX(20deg); } }
@keyframes priceFlipIn { from { opacity: 0; transform: translateY(-10px) rotateX(-20deg); } to { opacity: 1; transform: translateY(0) rotateX(0); } }
...
@keyframes shimmerSwipe { 0% { transform: translateX(-100%); } 100% { transform: translateX(300%); } }
.chip-shimmer { position: relative; overflow: hidden; }
.chip-shimmer .shimmer-line { position: absolute; top: 0; left: 0; width: 40%; height: 100%; background: linear-gradient(90deg, transparent, rgba(245, 200, 66, 0.3), transparent); transform: translateX(-100%); animation: shimmerSwipe 0.7s ease forwards; }

/* ── Card stagger ── */
@keyframes cardSlideIn { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
.p-card-stagger { opacity: 0; animation: cardSlideIn 0.35s ease forwards; }

/* ── Lock shake ── */
@keyframes lockShake { 0%, 100% { transform: translateX(0); } 15% { transform: translateX(-7px); } 30% { transform: translateX(7px); } 45% { transform: translateX(-5px); } 60% { transform: translateX(5px); } 75% { transform: translateX(-3px); } 90% { transform: translateX(3px); } }
.pw-icon.shake { animation: lockShake 0.5s ease; }

/* ── Unlock burst ── */
@keyframes burstRing { 0% { transform: scale(0.5); opacity: 0.9; } 100% { transform: scale(2.4); opacity: 0; } }
@keyframes burstRing2 { 0% { transform: scale(0.5); opacity: 0.6; } 100% { transform: scale(1.9); opacity: 0; } }
.burst-ring-wrap { position: relative; display: inline-flex; align-items: center; justify-content: center; }
.burst-r1, .burst-r2 { position: absolute; width: 52px; height: 52px; border-radius: 50%; border: 2px solid var(--gold); opacity: 0; pointer-events: none; }
...
/* ── Section glow ── */
@keyframes sectionGlow { 0% { box-shadow: 0 0 0 0 rgba(245, 200, 66, 0); } 30% { box-shadow: 0 0 0 8px rgba(245, 200, 66, 0.2); } 100% { box-shadow: 0 0 0 0 rgba(245, 200, 66, 0); } }
