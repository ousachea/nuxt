        ...
        <main class="main">
            ...
            <section class="section">
                ...
                <div v-if="purchases.length" class="purchases-grid">
                    <div v-for="(p, i) in purchases" :key="p.id" class="p-card">
                        <template v-if="editIdx !== i">
                            ...
                            <div class="pcard-rows">
                                ...
                                <div class="pcard-row">
                                    <span>{{ t.current }}</span>
                                    <span>${{ currentValue(p).toFixed(2) }}</span>
                                </div>
...
function exportCSV() {
    const hdr = ['Date', 'Weight', 'Unit', 'Paid', 'Current', 'G/L']
    const rows = purchases.value.map(p => [
        p.date, p.weight, p.unit,
        p.price.toFixed(2), currentValue(p).toFixed(2), gainLoss(p).toFixed(2)
    ])
