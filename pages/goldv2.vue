<template>
    <div class="app" :class="{ dark: isDark }">
        ...
        <header class="header">
            <div class="header-inner">
                ...
                <div class="header-controls">
                    <button class="ctrl-btn owner-btn" :class="{ unlocked: isOwner }"
                        @click="isOwner ? lockOwner() : openModal()"
                        :aria-label="isOwner ? 'Lock owner' : 'Owner login'"
                        :title="isOwner ? 'Lock owner session' : 'Owner login'">
                        <span>{{ isOwner ? '🔓' : '🔐' }}</span>
        ...
        <!-- Owner Login Modal — inside .app so CSS vars are inherited -->
        <transition name="modal-fade">
            <div v-if="showModal" class="modal-backdrop" @click.self="closeModal" role="dialog" aria-modal="true">
                <div class="modal-box">
                    <button class="modal-close" @click="closeModal" aria-label="Close">✕</button>

                    <div class="modal-icon-wrap" :class="{ burst: pwUnlockBurst }">
                        <div class="burst-r1"></div>
                        <div class="burst-r2"></div>
                        <div class="modal-icon" :class="{ shake: pwShake }">{{ modalIcon }}</div>
                    </div>

                    <div class="modal-title">{{ t.ownerLogin }}</div>
                    <div class="modal-sub">{{ t.ownerSub }}</div>

                    <transition name="fade" mode="out-in">
                        <div v-if="!modalSuccess" class="modal-form">
                            <input ref="pwFieldRef" v-model="pwInput" class="text-input" type="password"
                                :placeholder="t.enterPw" @keyup.enter="unlockOwner" autocomplete="current-password" />
                            <transition name="fade">
                                <p v-if="pwError" class="pw-error">{{ pwError }}</p>
                            </transition>
                            <button class="primary-btn full-btn" @click="unlockOwner">{{ t.unlock }}</button>
                        </div>
        ...
        t.live }}</button>
                    <button class="seg-btn" :class="{ active: priceSource === 'custom' }"
                        @click="priceSource = 'custom'">✦ {{ t.custom }}</button>
                </div>
                ...
                <div v-if="priceSource === 'api'" class="sub-panel">
                    ...
                    <p class="api-hint">{{ t.freeNoKey }} · <a href="https://www.goldapi.io/" target="_blank"
                            rel="noopener">goldapi.io</a> {{ t.asBackup }}</p>
                </div>
            ...
            <section class="card" id="purchases-section" :class="{ 'section-glow': purchasesGlow }">
                <div class="section-header">
                    ...
                    <div class="section-actions">
                        ...
                        <button v-if="isOwner" class="ghost-btn lock-btn" @click="lockOwner" :title="t.lock">🔒</button>
                        <input ref="csvInput" type="file" accept=".csv" hidden @change="importCSV" />
                    </div>
                </div>

                <!-- Owner active badge -->
                <div v-if="isOwner" class="owner-active-badge">
                    🔓 {{ t.ownerActive }}
                </div>

                <!-- Add button — open to everyone -->
...
// Guest purchases persist under this key for anyone not logged in as owner
const GUEST_KEY = 'gt4_guest'
...
function openModal() {
    ...
    pwInput.value = ''
    pwError.value = ''
    nextTick(() => pwFieldRef.value?.focus())
}
...
const translations = {
    en: {
        ...
        gain: 'Gain', loss: 'Loss', totalInvested: 'Invested', currentValue: 'Value Now',
        totalGainLoss: 'Total G/L', enterWeight: 'e.g. 2', enterPrice: 'e.g. 240',
        ...
        gram: 'Gram', li: 'Li', hun: 'Hun', chi: 'Chi', damlung: 'Damlung', troyOz: 'Troy Oz',
        fetchPriceFirst: 'Fetch a price first', offlineWarning: 'You are offline — prices may be outdated',
        noPurchases: 'No purchases yet. Tap + Add above.', lock: 'Lock owner session',
        owner: 'Owner', ownerLogin: 'Owner Login', ownerSub: 'Enter your password to view owner purchases',
        ownerActive: 'Viewing owner purchases', unlock: 'Unlock', enterPw: 'Enter password…',
        unlockSuccess: '✓ Unlocked — loading purchases…',
    },
    km: {
        ...
        gain: 'ចំណេញ', loss: 'ខាត', totalInvested: 'វិនិយោគ', currentValue: 'តម្លៃ',
        totalGainLoss: 'ចំណេញ/ខាត', enterWeight: 'ឧ. ២', enterPrice: 'ឧ. ២៤០',
        ...
        gram: 'ក្រាម', li: 'លី', hun: 'ហុន', chi: 'ជី', damlung: 'ដំឡឹង', troyOz: 'ត្រយ អោន',
        fetchPriceFirst: 'សូមទាញតម្លៃជាមុន', offlineWarning: '⚠ ក្រៅបណ្តាញ',
        noPurchases: 'មិនទាន់មានការទិញ។ ចុច + បន្ថែម', lock: 'ចាក់សោ',
        owner: 'ម្ចាស់', ownerLogin: 'ចូលជាម្ចាស់', ownerSub: 'បញ្ចូលពាក្យសម្ងាត់ដើម្បីមើលការទិញ',
        ownerActive: 'មើលការទិញជាម្ចាស់', unlock: 'ដោះសោ', enterPw: 'បញ្ចូលពាក្យសម្ងាត់…',
        unlockSuccess: '✓ បានដោះសោ — កំពុងផ្ទុក…',
    }
}
...
function switchMethod(m) { priceMethod.value = m; customPrice.value = null; goldPrice.value = null; save() }
...
async function unlockOwner() {
    if (!pwInput.value) { pwError.value = 'Enter a password'; triggerShake(); return }
    const hash = await sha256(pwInput.value)
    if (hash !== OWNER_PW_HASH) {
        pwError.value = 'Incorrect password'
        modalIcon.value = '❌'
        triggerShake()
        setTimeout(() => { modalIcon.value = '🔐' }, 700)
        return
    }
...
function lockOwner() {
    isOwner.value = false
    // Restore guest purchases after locking
    purchases.value = JSON.parse(localStorage.getItem(GUEST_KEY) || '[]')
}

function triggerShake() {
    pwShake.value = false
    nextTick(() => { pwShake.value = true; setTimeout(() => pwShake.value = false, 600) })
}
...
function scrollToPurchases() {
    ...
    if (el) {
        ...
        setTimeout(() => { purchasesGlow.value = true; setTimeout(() => purchasesGlow.value = false, 1000) }, 300)
    }
}
...
async function fetchPrice() {
    ...
    if (!ok && customApiUrl.value?.trim()) {
        try {
            const r = await fetch('https://www.goldapi.io/api/XAU/USD', {
                headers: { 'x-access-token': customApiUrl.value.trim(), Accept: 'application/json' }, mode: 'cors'
            })
            if (r.ok) { const d = await r.json(); if (d?.price) { goldPrice.value = parseFloat(d.price); ok = true } }
        } catch (_) { }
    }
    const cached = load()
    if (ok) {
    ...
    } else if (cached?.goldPrice) {
        goldPrice.value = cached.goldPrice
        lastUpdated.value = (cached.lastUpdated || '') + ' (cached)'
...
function save() {
    try {
        ...
        if (isOwner.value) {
            ...
            localStorage.setItem(GUEST_KEY, JSON.stringify(purchases.value))
        }
    } catch { }
}
...
onMounted(() => {
    ...
    // Load guest purchases by default
    purchases.value = JSON.parse(localStorage.getItem(GUEST_KEY) || '[]')
...
/* ── Modal — inside .app so all CSS vars resolve correctly ── */
.modal-backdrop {
    position: fixed;
    inset: 0;
    z-index: 200;
...
.modal-box {
    /* Explicit fallback colors so the box is never transparent */
    background: #13131C;
...
.modal-icon-wrap.burst .burst-r1 {
    animation: burstRing 0.65s ease-out forwards;
}

.modal-icon-wrap.burst .burst-r2 {
...
.p-card-stagger {
    opacity: 0;
    animation: cardSlideIn 0.35s ease both forwards;
}
